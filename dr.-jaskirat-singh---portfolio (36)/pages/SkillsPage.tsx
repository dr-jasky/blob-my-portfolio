...
const ExpertiseCard: React.FC<{ item: ExpertiseArea, delay: number }> = ({ item, delay }) => (
  <div 
    className="glass-card p-6 rounded-xl hover-card animate-fadeIn h-full flex flex-col" 
    style={{animationDelay: `${delay}s`}}
  >
    <div className="flex items-center mb-4">
      <div className={`w-12 h-12 rounded-full ${item.iconBgColor} flex items-center justify-center mr-4 shadow-md transition-transform duration-300 group-hover:scale-110`}>
    ...
    <div className="flex flex-wrap gap-2 mt-auto">
      {item.tags.map(tag => (
        <span key={tag} className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${item.tagBgColor} ${item.tagTextColor} shadow-sm hover:brightness-125 transition-all`}>
          {tag}
        </span>
      ))}
    </div>
...
const SkillCategoryCard: React.FC<{ category: SkillCategory, delay: number }> = ({ category, delay }) => (
  <div 
    className="p-6 md:p-8 bg-dark-secondary rounded-xl shadow-xl border-l-4 border-neon-pink hover:shadow-neon-glow-pink hover:border-neon-pink/70 transition-all duration-300 transform hover:-translate-y-1.5 animate-fadeIn"
    style={{ animationDelay: `${delay}s`}}
  >
