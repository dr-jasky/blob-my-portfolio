...
const HeroSection: React.FC = () => {
  ...
  return (
  <div className="min-h-[calc(85vh-80px)] md:min-h-[calc(75vh-80px)] flex flex-col items-center justify-center text-center bg-dark-primary py-12 md:py-16 relative overflow-hidden"> {/* bg-dark-primary can be var(--dark-primary) */}
    <div className="absolute inset-0 opacity-[0.03] z-0"> {/* Keep subtle pattern or remove if too busy with particles */}
      <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="heroPattern" patternUnits="userSpaceOnUse" width="70" height="70" patternTransform="scale(1) rotate(30)">
             {/* Pattern can be simplified or made more neon */}
          </pattern>
        </defs>
    ...
    <div className="relative z-10 px-4 flex flex-col items-center">
      ...
      />
      <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-text-light-primary mb-3 animate-fadeIn" style={{animationDelay: '0.3s'}}>
      ...
      <p className="text-md text-text-light-secondary mb-5 animate-fadeIn" style={{animationDelay: '0.55s'}}>
         ...
         <div className="mt-10 mb-8 md:mb-12 animate-fadeIn" style={{ animationDelay: `${keyMetricsContainerDelay}s`}}>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-5 max-w-4xl mx-auto">
            {personalInfoData.keyStats.map((stat, index) => (
              <div 
                key={stat.id} 
                className="bg-dark-secondary/80 p-5 rounded-lg text-center shadow-xl border border-neon-blue/40 transition-all duration-300 hover:border-neon-blue hover:shadow-neon-glow-blue hover:bg-dark-tertiary/80 animate-fadeIn transform hover:scale-[1.05]" // bg-dark-secondary, border-neon-blue, shadow-neon-glow-blue
              ...
              >
                ...
                <div className="text-3xl sm:text-4xl font-bold text-text-light-primary">
                  {stat.value}{stat.suffix && <span className="text-neon-blue">{stat.suffix}</span>}
                </div>
                <p className="text-sm sm:text-base text-text-light-secondary mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
...
const ImpactCard: React.FC<{ title: string; linkTo: string; icon: string; description: string; colorClass: string; delay: number; }> = ({ title, linkTo, icon, description, colorClass, delay }) => {
  const borderColorClass = 
    ...
    colorClass === 'neon-green' ? 'border-neon-green' : 'border-neon-purple'; // Added purple
  
  const textColorClass = 
    colorClass === 'neon-blue' ? 'text-neon-blue' :
    colorClass === 'neon-pink' ? 'text-neon-pink' :
    colorClass === 'neon-green' ? 'text-neon-green' : 'text-neon-purple';

  const shadowHoverClass = 
    colorClass === 'neon-blue' ? 'hover:shadow-neon-glow-blue' :
    colorClass === 'neon-pink' ? 'hover:shadow-neon-glow-pink' :
    colorClass === 'neon-green' ? 'hover:shadow-neon-glow-green' : 'hover:shadow-neon-glow-purple';
  ...
  return (
    ...
    >
      <div className={`flex items-center ${textColorClass} mb-4`}>
        <i className={`${icon} text-4xl mr-4 transition-transform duration-300 group-hover:scale-115 group-hover:rotate-[-6deg] rounded-full p-2.5 bg-dark-primary/70`}></i> {/* bg-dark-primary */}
        <h3 className={`text-2xl font-semibold text-text-light-primary group-hover:${textColorClass} transition-colors`}>{title}</h3>
      </div>
      <p className="text-text-light-secondary text-sm leading-relaxed flex-grow">{description}</p>
...
const PublicationPreviewCard: React.FC<{ pub: Publication }> = ({ pub }) => (
  <div className="bg-dark-secondary/80 p-5 rounded-lg shadow-lg hover:shadow-neon-glow-blue transition-all duration-300 border-l-4 border-neon-blue hover:border-neon-pink transform hover:-translate-y-2 hover:scale-[1.025] focus-within:border-neon-pink focus-visible-outline h-full flex flex-col">
    ...
    <p className="text-xs text-text-light-secondary mb-1 italic truncate">{pub.authors}</p>
    <p className="text-xs text-text-light-secondary mb-2 truncate">
      {pub.source}, {pub.year}
    </p>
   ...
   <div className="bg-dark-secondary/80 rounded-lg shadow-xl overflow-hidden group flex flex-col h-full hover:shadow-neon-glow-pink transition-all duration-300 transform hover:scale-[1.035] focus-visible-outline">
    {pub.featuredImageUrl ? (
      <div className="relative overflow-hidden h-56"> 
        ...
        <div className="w-full h-56 bg-dark-primary/70 flex items-center justify-center border-b border-neon-blue/40"> {/* bg-dark-primary */}
    ...
    <div className="p-5 flex-grow flex flex-col">
      ...
      <p className="text-sm text-text-light-secondary mb-3 line-clamp-3 flex-grow">{pub.insightSnippet || pub.summary || "Click to read more about this research."}</p>
      {pub.impactMetrics && pub.impactMetrics.length > 0 && (
        <div className="mb-3">
...
const TestimonialCard: React.FC<{ testimonial: Testimonial }> = ({ testimonial }) => (
  <div className="bg-dark-secondary/80 p-6 rounded-lg shadow-xl border-t-4 border-neon-pink/60 flex flex-col items-center text-center transform transition-all duration-300 hover:scale-105 hover:shadow-neon-glow-pink focus-visible-outline"> {/* bg-dark-secondary */}
    ...
    {!testimonial.avatarUrl && (
      <div className="w-24 h-24 rounded-full mx-auto mb-5 border-2 border-neon-pink bg-dark-primary/70 flex items-center justify-center shadow-lg"> {/* bg-dark-primary */}
        <i className="fas fa-user-tie text-4xl text-neon-pink opacity-75"></i>
      </div>
    )}
    <blockquote className="mb-4 flex-grow">
      <p className="text-text-light-secondary italic text-md leading-relaxed">
    ...
    <div className="mt-auto pt-4">
      ...
      <p className="text-xs text-text-light-secondary">{testimonial.authorTitle}</p>
    </div>
  </div>
);
...
export const HomePage: React.FC = () => {
  ...
  return (
    <div className="animate-fadeIn"> 
      ...
      <Section title="My Impact Areas" id="impact-areas" className="bg-dark-primary/60" subtitle="Leveraging expertise to drive meaningful change and foster innovation."> {/* bg-dark-primary */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <ImpactCard title="Key Expertise Areas" linkTo="/#skills" icon="fas fa-cogs" description="Deep expertise spanning quantitative/qualitative analysis, fintech, financial inclusion, project management, and cross-cultural communication." colorClass="neon-purple" delay={0.5}/> {/* Changed to neon-purple for variety */}
        </div>
      </Section>
      
      {featuredPublications.length > 0 && (
        <Section title="Recent Publications" id="featured-publications" className="bg-dark-secondary/80" subtitle="Highlights from my most impactful and recent contributions."> {/* bg-dark-secondary */}
          ...
          <div className="text-center mt-12 animate-fadeIn" style={{animationDelay: `${0.2 + featuredPublications.length * 0.1}s`}}>
              <Link 
                to="/#research"
                onClick={(e) => { e.preventDefault(); document.getElementById('research')?.scrollIntoView({behavior:'smooth', block:'start'});}}
                className="inline-block gradient-bg text-dark-primary font-semibold py-3.5 px-10 rounded-full shadow-lg hover:opacity-90 transition-all duration-300 transform hover:scale-105 text-md focus-visible-outline"
              >
      ...
      {researchHighlights.length > 0 && (
        <Section title="Research Spotlights" id="research-highlights" className="bg-dark-primary/60" subtitle="Key insights and takeaways from my impactful research."> {/* bg-dark-primary */}
      ...
      {testimonialsData.length > 0 && (
        <Section title="What Others Say" id="testimonials" className="bg-dark-secondary/80" subtitle="Feedback from collaborators and partners."> {/* bg-dark-secondary */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
