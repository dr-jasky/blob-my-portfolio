...
const KeyMetrics: React.FC<{ stats: KeyStat[] }> = ({ stats }) => {
  ...
  // Base delay calculation moved to HeroSection for coordination
  return (
    <div className="mt-10 mb-8 md:mb-12">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 max-w-3xl mx-auto">
        {stats.map((stat, index) => (
          <div 
            key={stat.id} 
            className="bg-dark-secondary/60 p-4 rounded-lg text-center shadow-lg border border-neon-blue/25 transition-all duration-300 hover:border-neon-blue hover:shadow-neon-glow-blue hover:bg-dark-tertiary animate-fadeIn transform hover:scale-[1.03]"
            style={{ animationDelay: `${1.2 + index * 0.15}s`}} // Default delay, HeroSection might override
          >
...
const HeroSection: React.FC = () => {
  const summaryDelay = 0.7;
  const keywordsTitleDelay = summaryDelay + 0.2; 
  ...
  const keyMetricsContainerDelay = keyMetricsBaseDelay - 0.1; // Wrapper fades in before first child
  ...
  <div className="min-h-[calc(85vh-80px)] md:min-h-[calc(75vh-80px)] flex flex-col items-center justify-center text-center bg-dark-primary py-12 md:py-16 relative overflow-hidden">
      ...
      <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="heroPattern" patternUnits="userSpaceOnUse" width="70" height="70" patternTransform="scale(1) rotate(30)">
            {/* Circle (dots) and Path (cross-lines) elements removed for a clean background */}
          </pattern>
        </defs>
    ...
    <div className="relative z-10 px-4 flex flex-col items-center">
      ...
      <p className="text-md sm:text-lg text-light-secondary max-w-2xl mx-auto mb-8 animate-fadeIn" style={{animationDelay: `${summaryDelay}s`}}>
        {personalInfoData.professionalSummary ? personalInfoData.professionalSummary.substring(0, 160) + '...' : ''}
      </p>
      
      {personalInfoData.currentFocusKeywords && personalInfoData.currentFocusKeywords.length > 0 && (
        <div className="my-6 md:my-8 max-w-3xl mx-auto w-full">
          <p className="text-md text-neon-pink mb-4 font-semibold animate-fadeIn" style={{ animationDelay: `${keywordsTitleDelay}s` }}>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {personalInfoData.currentFocusKeywords.map((keyword, index) => (
              <span
                key={keyword}
                className="px-4 py-2 bg-dark-secondary border border-neon-blue/40 text-neon-blue rounded-full text-xs sm:text-sm font-medium transition-all duration-300 hover:bg-neon-blue/25 hover:shadow-neon-glow-blue hover:scale-105 cursor-default animate-fadeIn"
      ...
      {/* KeyMetrics: Pass calculated delays */}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-6 animate-fadeIn" style={{animationDelay: `${actionButtonsDelay}s`}}>
        <Link 
          ...
          className="inline-block bg-neon-blue hover:bg-opacity-85 text-dark-primary font-semibold py-3.5 px-10 rounded-lg shadow-neon-glow-blue transition-all duration-300 transform hover:scale-105 text-lg focus-visible-outline"
        >
...
const ImpactCard: React.FC<{ title: string; linkTo: string; icon: string; description: string; colorClass: string; delay: number; }> = ({ title, linkTo, icon, description, colorClass, delay }) => {
  ...
  return (
    <Link 
      to={linkTo} 
      className={`group block p-6 bg-dark-secondary rounded-xl shadow-xl ${shadowHoverClass} transition-all duration-300 transform hover:-translate-y-2 border-2 ${borderColorClass}/30 hover:${borderColorClass} animate-fadeIn focus-visible-outline`}
      style={{ animationDelay: `${delay}s`}}
    >
      <div className={`flex items-center ${textColorClass} mb-4`}>
        <i className={`${icon} text-4xl mr-4 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-[-5deg] rounded-full p-2 bg-dark-primary`}></i>
      ...
      <span className={`mt-4 inline-block text-sm font-medium ${textColorClass} group-hover:underline`}>Learn More &rarr;</span>
    </Link>
  );
};
...
const PublicationPreviewCard: React.FC<{ pub: Publication }> = ({ pub }) => (
  <div className="bg-dark-secondary p-5 rounded-lg shadow-lg hover:shadow-neon-glow-blue transition-all duration-300 border-l-4 border-neon-blue hover:border-neon-pink transform hover:-translate-y-1.5 hover:scale-[1.02] focus-within:border-neon-pink focus-visible-outline">
    ...
    {pub.impactMetrics && pub.impactMetrics.length > 0 && (
      <div className="mt-2 mb-1">
        {pub.impactMetrics.slice(0, 2).map(metric => (
          <span key={metric.name+metric.value} className={`inline-block text-xs font-medium mr-2 mb-1 px-2 py-0.5 rounded-full ${metric.value.toString().startsWith('Q1') ? 'bg-neon-green/20 text-neon-green' : 'bg-neon-blue/20 text-neon-blue'}`}>
           {metric.icon && <i className={`${metric.icon} mr-1`}></i>}{metric.name && `${metric.name}: `}{metric.value}
          </span>
        ))}
      </div>
    )}
    <Link to={`/#research`} onClick={(e) => {e.preventDefault(); const el = document.getElementById(`pub-${pub.id}`); if(el) el.scrollIntoView({behavior: 'smooth', block: 'center'}); }} className="text-xs text-neon-pink hover:underline focus-visible-outline rounded-sm">View Details &rarr;</Link>
  </div>
);

const ResearchHighlightCard: React.FC<{ pub: Publication }> = ({ pub }) => (
   <div className="bg-dark-secondary rounded-lg shadow-xl overflow-hidden group flex flex-col h-full hover:shadow-neon-glow-pink transition-all duration-300 transform hover:scale-[1.03] focus-visible-outline">
    {pub.featuredImageUrl ? (
      <div className="relative overflow-hidden h-48">
        ...
        <div className="w-full h-48 bg-dark-primary flex items-center justify-center border-b border-neon-blue/30">
            <i className="fas fa-atom text-5xl text-neon-blue opacity-50 group-hover:animate-pulseGlow [--tw-shadow-color:theme('colors.neon-blue')] group-hover:opacity-80"></i> 
        </div>
    )}
      ...
      {pub.impactMetrics && pub.impactMetrics.length > 0 && (
        <div className="mb-3">
          {pub.impactMetrics.map(metric => (
            <span key={metric.name+metric.value} className={`inline-flex items-center text-xs font-semibold mr-2 mb-1 px-2.5 py-1 rounded-full ${metric.value.toString().startsWith('Q1') ? 'bg-neon-green/80 text-dark-primary' : 'bg-neon-blue/80 text-dark-primary'}`}>
      ...
      <div className="mt-auto">
        {pub.doiLink ? (
          ...
          <a href={pub.link} target="_blank" rel="noopener noreferrer" className="text-sm text-neon-pink hover:underline font-medium group-hover:text-neon-blue transition-colors focus-visible-outline rounded-sm">
           ...
           <Link to={`/#research`} onClick={(e) => {e.preventDefault(); const el = document.getElementById(`pub-${pub.id}`); if(el) el.scrollIntoView({behavior: 'smooth', block: 'center'}); }} className="text-sm text-neon-pink hover:underline font-medium group-hover:text-neon-blue transition-colors focus-visible-outline rounded-sm">
...
const TestimonialCard: React.FC<{ testimonial: Testimonial }> = ({ testimonial }) => (
  <div className="bg-dark-secondary p-6 rounded-lg shadow-xl border-t-4 border-neon-pink/50 flex flex-col items-center text-center transform transition-all duration-300 hover:scale-105 hover:shadow-neon-glow-pink focus-visible-outline">
...
export const HomePage: React.FC = () => {
  ...
  // Fill remaining spots in researchHighlights from featuredPublications if needed
        ...
        <Section title="Recent Publications" id="featured-publications" className="bg-dark-secondary" subtitle="Highlights from my most impactful and recent contributions.">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredPublications.map((pub,idx) => (
              <div key={pub.id} className="animate-fadeIn" style={{animationDelay: `${0.2 + idx * 0.1}s`}}>
                <PublicationPreviewCard pub={pub} />
              </div>
            ))}
          </div>
          <div className="text-center mt-10 animate-fadeIn" style={{animationDelay: `${0.2 + featuredPublications.length * 0.1}s`}}>
              <Link 
                to="/#research"
                onClick={(e) => { e.preventDefault(); document.getElementById('research')?.scrollIntoView({behavior:'smooth', block:'start'});}}
                className="inline-block bg-neon-blue hover:bg-opacity-85 text-dark-primary font-semibold py-3 px-8 rounded-lg shadow-neon-glow-blue transition-all duration-300 transform hover:scale-105 text-md focus-visible-outline"
              >
