...
const ExperienceItemCard: React.FC<{ item: ExperienceType, iconColorClass: string, delay: number }> = ({ item, iconColorClass, delay }) => (
  ...
  > 
    <div className="hidden sm:block absolute left-[23px] top-0 bottom-0 w-[3px] bg-slate-700/70 -z-10 timeline-line rounded-full"></div> {/* Thicker line */}
    
    <div className="flex-shrink-0 mr-6">
      <div className={`w-12 h-12 rounded-full ${iconColorClass} flex items-center justify-center text-white relative z-10 timeline-item shadow-md hover:brightness-125 transition-all`}>
        <i className={`${item.icon || 'fas fa-briefcase'} text-xl`}></i>
      </div>
    </div>
    
    <div className="pb-10 w-full"> 
      <div className={`glass-card p-5 rounded-lg shadow-md border-l-4 ${iconColorClass.replace('bg-', 'border-').replace('-700', '-500')} hover:shadow-lg hover:border-opacity-100 transition-all duration-300`}>
        <div className="flex flex-col sm:flex-row justify-between items-start mb-1">
          <h3 className="text-xl font-bold text-light">{item.role}</h3>
          <span className={`text-xs sm:text-sm ${iconColorClass.replace('bg-', 'bg-').replace('-700', '-800')} ${iconColorClass.replace('bg-', 'text-').replace('-700', '-100')} px-3 py-1 rounded-full mt-1 sm:mt-0 whitespace-nowrap font-medium shadow-sm`}>
            {item.period}
          </span>
        </div>
        <p className={`${iconColorClass.replace('bg-', 'text-').replace('-700','-300')} mt-0.5 text-sm font-medium`}>{item.organization}</p>
        <p className="text-xs text-text-darker-muted mb-2">{item.location}</p>
        <ul className="mt-4 space-y-3 text-text-muted text-sm list-none pl-1"> {/* Removed list-disc for custom icons */}
          {item.descriptionPoints.map((point, index) => (
            <li key={index} className="flex items-start">
               <i className={`fas fa-check-circle ${iconColorClass.replace('bg-','text-').replace('-700','-400')} mt-1 mr-2.5 flex-shrink-0`}></i>
              <span>{point}</span>
            </li>
          ))}
        </ul>
...
export const ExperiencePage: React.FC = () => {
  const experienceIconColors = ['bg-primary-dark', 'bg-secondary', 'bg-accent']; // Cycle for experience cards
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12"> {/* Increased gap */}
                ...
                <div className="lg:col-span-2">
                    <div className="relative space-y-10">
                        <div className="hidden sm:block absolute left-[23px] top-0 bottom-0 w-[3px] bg-slate-700/70 -z-10 rounded-full"></div>
                ...
                <div className="lg:col-span-1 animate-fadeIn" style={{animationDelay: '0.3s'}}>
                    <div className="glass-card p-8 rounded-xl shadow-xl h-full">
                        ...
                        <div>
                            ...
                            {specializedSkillsData.academicSkills.map((skill, idx) => (
                                ...
                                <i className={`${skill.icon || 'fas fa-check-circle text-green-500'} mr-2.5 w-4 text-center`}></i>
